...
class ApexSmoother:
    ...
    @staticmethod
    def smooth(signal: list, median_kernel_size: int = 5) -> list:
        """
        Melakukan perataan sinyal menggunakan Savitzky-Golay filter
        diikuti dengan median filter untuk menghilangkan spike noise.

        Pipeline: raw → Savitzky-Golay → Median Filter

        Args:
            signal (list): Sinyal input yang akan diratakan.
            median_kernel_size (int): Ukuran kernel untuk median filter (harus ganjil).
        ...
        print(length, window_length, polyorder)
        ...
        # Median filter untuk menghilangkan spike noise
        smoothed = medfilt(smoothed, kernel_size=median_kernel_size)
